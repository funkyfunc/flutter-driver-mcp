...
function parseDaemonEvents(raw: string): void {
	for (const line of raw.split("\n")) {
		...
		try {
			let events = JSON.parse(trimmed) as
				| FlutterDaemonEvent
				| FlutterDaemonEvent[];
...
async function handleTakeScreenshot(args: ToolArgs): Promise<ToolResponse> {
	...
	if (screenshotType !== "device")
		screenshotArgs.push(`--vm-service-url=${s.observatoryUri}`);
		...
		if (!content.includes("com.apple.security.network.client")) {
			...
			if (autoFix) {
				...
				if (idx !== -1) {
					...
					report.push(
						"⚠️ Could not auto-fix entitlements (structure mismatch).",
					);
				}
			}
		} else {
			...
			if (autoFix) {
				try {
					...
					if (debugContent.includes("</manifest>")) {
						...
						report.push(
							"✅ Added INTERNET permission to debug AndroidManifest.xml.",
						);
					} else {
						report.push(
	...
	try {
		...
		if (!gitignore.includes("integration_test/mcp_harness.dart")) {
			if (autoFix) {
				...
				report.push(
					"✅ Added 'integration_test/mcp_harness.dart' to .gitignore.",
				);
			} else {
				report.push(
					"❌ Missing 'integration_test/mcp_harness.dart' in .gitignore.",
				);
				success = false;
			}
		}
