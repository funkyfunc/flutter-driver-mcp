#!/bin/sh
# Verify harness.dart Dart syntax by running dart analyze inside the test_app Flutter project

HARNESS_SRC="$(dirname "$0")/../src/harness.dart"
...
cd "$TEST_APP" && dart analyze integration_test/mcp_harness_verify.dart
EXIT=$?
